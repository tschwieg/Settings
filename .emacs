...
(custom-set-variables
 ...
 '(package-selected-packages
   (quote
    (preview-latex preview-mode ess noweb-mode noweb-mdoe no-web-mode polymode org-mode auto-complete julia-mode pyenv-mode font-lock-studio)))
...
(defun pdfWaiterFunc (x )
  (start-process-shell-command "pdfWaiter" "pdfWaiterOutput" "function pdfView {
        if [[ ! -a /tmp/pipe.pdf ]] then
         	mkfifo /tmp/pipe.pdf;
        fi
        i=0;
	while true;do
		cat /tmp/pipe.pdf > /tmp/.recentPDF${i}.pdf;
		emacsclient --eval \"(find-file \\\"/tmp/.recentPDF${i}.pdf\\\")\";
                let i=i+1;
	done
}
...
(add-to-list 'load-path "~/ob-julia")
(org-babel-do-load-languages
 'org-babel-load-languages
 '((emacs-lisp . t) (julia . t) (python . t) (R . t)))
